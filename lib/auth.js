...
/**
 * Signed-in user, even if their email isn't verified yet. Only for the
 * verification endpoints themselves; everything else uses requireAuth().
 */
...
/** Signed-in user with a verified email (clients must confirm their address first). */
export async function requireAuth() {
  const user = await requireSession();
  if (user.emailVerified === false) {
    throw new ApiError(403, "Please verify your email address to continue.", { code: "EMAIL_NOT_VERIFIED" });
  }
  return user;
}
