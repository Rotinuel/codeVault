...
export const messages = {
  ...
  ticketApproved({ name, payout, currency, rewardsEnabled, approved, target, percent, monthLabel, usableIn }) {
    const reward = !rewardsEnabled
      ? "Thanks for sharing your win!"
      : approved >= target
        ? `That's ${approved} approved upload${approved === 1 ? "" : "s"} for ${monthLabel} — target reached! You'll get ${percent}% off one subscription payment in ${usableIn}.`
        : `That's ${approved} of ${target} approved uploads for ${monthLabel}. Reach ${target} to get ${percent}% off one payment in ${usableIn}.`;
    return {
      event: NOTIFICATION_EVENTS.WINNING_TICKET_REVIEWED,
      type: NOTIFICATION_TYPES.SUBSCRIPTION,
      title: "Winning ticket approved 🎉",
      message: `Your ${formatCurrency(payout, currency)} winning ticket was verified. ${reward}`,
      whatsapp: `Congrats ${name}! 🎉 Your ${formatCurrency(payout, currency)} winning ticket has been verified.\n\n${reward}\n\n${link("/dashboard/winning-tickets")}`,
      params: [name, formatCurrency(payout, currency), reward],
      link: "/dashboard/winning-tickets",
    };
  },

  ticketRejected({ name, payout, currency, reason }) {
    const why = reason ? ` Reason: ${reason}` : "";
    return {
      event: NOTIFICATION_EVENTS.WINNING_TICKET_REVIEWED,
      type: NOTIFICATION_TYPES.SUBSCRIPTION,
      title: "Winning ticket not approved",
      message: `We couldn't verify your ${formatCurrency(payout, currency)} ticket.${why}`,
      whatsapp: `Hi ${name}, we couldn't verify your ${formatCurrency(payout, currency)} winning ticket.${why}\n\nYou can upload a clearer image or a different ticket: ${link("/dashboard/winning-tickets")}`,
      params: [name, formatCurrency(payout, currency), reason || "-"],
      link: "/dashboard/winning-tickets",
    };
  },
