...
import { applyDiscount } from "../ticket-rewards.js";
import { getRewardStatus, markRewardRedeemed } from "./winning-tickets.js";
...
export async function initializePayment({ user, planId, intent = "subscribe", request }) {
  ...
  // Winning-ticket reward: last month's target met and not yet used on a paid
  // payment. Computed here from the database, never taken from the client.
  const reward = await getRewardStatus(user._id).catch((error) => {
    console.error("[payments] reward lookup failed:", error?.message);
    return { discount: 0, rewardMonth: null };
  });
  const discount = reward.discount || 0;

  if (pending && pending.intent === type && (pending.discountPercent || 0) === discount) {
    return { authorizationUrl: pending.authorizationUrl, reference: pending.reference, reused: true };
  }

  const amount = applyDiscount(plan.price, discount);
  ...
  const payment = await Payment.create({
    ...
    rewardMonth: discount ? reward.rewardMonth : null,
    currency,
    status: PAYMENT_STATUS.PENDING,
    intent: type,
    ipAddress,
  ...
  try {
    const tx = await initializeTransaction({
      email: user.email,
      amount, // discounted amount; validateGatewayTransaction checks Paystack charged exactly this
      currency,
      reference,
      ...
      metadata: {
        ...
        custom_fields: [
          ...
          ...(discount
            ? [{ display_name: "Winning-ticket discount", variable_name: "discount", value: `${discount}% off ${plan.price} (${reward.rewardMonth} target)` }]
            : []),
        ],
      },
    });
...
export async function fulfillPayment(reference, { source = "callback", cancelled = false, expectedUserId = null } = {}) {
  ...
  await markRewardRedeemed(claimed, paidAt).catch((error) => console.error("[payments] reward bookkeeping failed:", error?.message));
